    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <div class="bg-white rounded-lg shadow-sm border border-border-light p-6">
                <div class="text-sm font-medium text-text-secondary">総蔵書数</div>
                <div class="text-3xl font-bold text-text-primary mt-2">{{ $totalBooks }}</div>
            </div>
            <div class="bg-white rounded-lg shadow-sm border border-border-light p-6">
                <div class="text-sm font-medium text-text-secondary">貸出中</div>
                <div class="text-3xl font-bold text-text-primary mt-2">{{ $loanedBooks }}</div>
            </div>
            <a href="{{ route('loans.overdue') }}" class="bg-white rounded-lg shadow-sm border border-border-light p-6 hover:border-border-neutral transition-colors">
                <div class="text-sm font-medium text-text-secondary">延滞中</div>
                <div class="text-3xl font-bold text-text-primary mt-2 {{ $overdueBooks > 0 ? 'underline decoration-red-400 decoration-2 underline-offset-4' : '' }}">{{ $overdueBooks }}</div>
            </a>
            <div class="bg-white rounded-lg shadow-sm border border-border-light p-6">
                <div class="text-sm font-medium text-text-secondary">登録ユーザー数</div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-medium text-text-secondary uppercase">書籍</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-text-secondary uppercase">借主</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-text-secondary uppercase">貸出日</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-text-secondary uppercase">状態</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200">
                            @foreach($recentLoans as $loan)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-sm text-text-secondary">{{ $loan->user->name }}</td>
                                    <td class="px-6 py-4 text-sm text-text-secondary">{{ $loan->borrowed_at->format('Y/m/d') }}</td>
                                    <td class="px-6 py-4">
                                        @if($loan->returned_at)
