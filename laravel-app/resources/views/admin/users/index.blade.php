            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-border-light">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-text-secondary uppercase tracking-wider">ユーザー</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-text-secondary uppercase tracking-wider">メールアドレス</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-text-secondary uppercase tracking-wider">権限</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-text-secondary uppercase tracking-wider">貸出中</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-text-secondary uppercase tracking-wider">登録日</th>
                        ...
                        @foreach($users as $user)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    @else
                                        <span class="text-xs font-medium text-text-secondary">一般ユーザー</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-text-secondary">{{ $user->loans_count }}冊</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-text-secondary">{{ $user->created_at->format('Y/m/d') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($user->id !== auth()->id())
                                        @php
