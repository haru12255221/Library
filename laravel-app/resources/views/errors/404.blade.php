<x-app-layout>
    <div class="max-w-lg mx-auto px-4 py-20 text-center">
        <div class="mb-6">
            <span class="text-8xl font-bold text-primary/30">404</span>
        </div>
        <h1 class="text-2xl font-bold text-text-primary mb-3">ページが見つかりません</h1>
        <p class="text-text-secondary mb-8">お探しのページは移動または削除された可能性があります。</p>
        <a href="{{ route('books.index') }}"
           class="inline-flex items-center gap-2 px-6 py-3 bg-primary hover:bg-primary-hover text-white rounded-lg font-medium transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path>
            </svg>
            トップページに戻る
        </a>
    </div>
</x-app-layout>
